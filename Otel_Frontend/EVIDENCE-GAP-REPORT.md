...
- 建議修正：保留「integration pattern 不是現況」的界線，改為「Foreman bounded scan 未找到 generic shared Button/Link；未來 wrapper/portal 與其他 application 仍需 targeted verification」。main 後續重建的 Lesson 2 已不含原本 stale sentence；bounded scan 結果保留於 durable evidence，未被擴張成全 application 現況。
...
- Matt Pocock learner-facing `reference/` 仍只作為 learner-facing Reference，沒有被搬動或改作 source storage；整合時只更新因 `source-documents/` 遷移而失效的 material links。

...
BASE = "https://c0004512000.github.io/Learning/Skill/Learning/Test/v4/"
...
def run_flow(page) -> None:
    ...
    assert page.locator(".progress-summary").count() == 1
    assert "M1 · Linux 隔離基礎" in page.locator(".progress-summary").inner_text()
    ...
    assert "補強路徑" not in body_text
    assert "補強完成後回到" not in body_text
